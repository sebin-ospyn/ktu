...
public class ViewAttendanceInternal extends SeleniumBase{
	//**************************************************************************************
...
//	public static List<String>IneligibleType = new ArrayList<String>();
	...
	public ViewAttendanceInternal(WebDriver driver) {
		super(driver);
	...
	public String[] CoursecodeFacultyId() throws InterruptedException {
		...
		try {
			...
			for( int j = 1; j<= CourseSize;j++)
			{
				...
				Branchname = driver.findElement(By.xpath("(//li[@class='list-group-item'])[3]")).getText();
				...
				Branch = StringUtils.substringAfter(Branchname, "Branch\n");
	...
	public void StaffAdvisor(String advisor) throws InterruptedException {
		...
		try {
			...
			for (int i = 1; i < rows.size(); i++)
			{
				...
				System.out.println(Branch);
				if(cellBranch.getText().equals(Branch))
				{

					for(int j=0;j<batchnames.size();j++)

						if(cellBatch.getText().equals(batchnames.get(i)))
						{
		...
		try {
			...
			for (int j = 0; j < rws.size(); j++) {
				...
				for( int k = 0; k< CourseSize;k++)
				{
					...
					if(courses[1].equals(courseCodes.get(k))&&Branch.equals(Branchdetails.get(k))&&Batch.equals(batchnames.get(k)))
					{
						System.out.println("**********************");
						System.out.println("Course code is "+courseCodes.get(k));
				...
				WebElement element = driver.findElement(By.xpath("//*[@class='table table-responsive table-bordered']/tbody/tr["+(j+1)+"]/td[8]/a[1]"));
						...
						System.out.println("entering attendance is "+Attendance.get(0));
 						Attendance.get(0).sendKeys(maxvalue);
 						System.out.println(maxvalue);
						System.out.println("attendance iss "+Attendance.get(0).getText());
...
//	*******************************************************************************************************************
	...
	public void loginasStaffAdvisor() throws InterruptedException {

		//Enters the StaffAdvisor
		username.sendKeys(StaffAdvisorId.get(0));
		Thread.sleep(1000);
