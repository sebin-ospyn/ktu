...
public class ViewMenstrualattendanceInternalFinal extends SeleniumBase{
	...
	public ViewMenstrualattendanceInternalFinal(WebDriver driver) throws IOException {
//		Initializing the driver
		...
		//Getting the sheet
		sheet = workbook.getSheet("AttendanceInternal");
...
//	public static List<String>Branchnames=new ArrayList<String>();
...
// 			Maximize the window
			...
			Thread.sleep(1000);
			WebElement studentmodule = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@id='4']")));
//			WebElement studentmodule = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@href='/university/eu/stu/studentDetailsListing.htm']")));
			studentmodule.click();
...
//		Reading male student details from student details page
...
// 		Iterating through student details listing page(If more than one page)
			...
			for (int j = 0; j < rws.size(); j++) {
				...
				for( int k = 0; k< CourseSize;k++)
				{
					...
					if(courseCode.equals(courseCodes.get(k))&&Branch.equals(Branchdetails.get(k))&&Batch.equals(batchnames.get(k))&&flags.get(k)==0)
					{
						...
						js.executeScript("arguments[0].click();", getSaveAttendancebutton());
...
//						getSaveAttendancebutton().click();
						...
						//*************************************************************
						js.executeScript("arguments[0].click();", getMarkAsComplete());
...
//				WebElement element = driver.findElement(By.xpath("(//table[@id='programDetail']//tbody//tr["+(i+1)+"]//td[text()='Submitted by Faculty']//following::a[1])"));
				...
				// Scroll to the bottom of the page(Some elements may not be visible initially and require scrolling to come into view.)So code added
